...
<template>
  <div id="message">
    <div class="wrapper" style="width: 60%;margin: 0 auto;">
      <div class="title" style="font-size: 24px;font-weight: bold;margin-bottom: 30px;">留言</div>
      <div class="title1">
      ...
      <div class="btn">
        <el-button type="primary" @click="submit()" style="font-size: 20px;">提交留言</el-button>
      </div>
      <div class="all">
        <ul class="msglist">
          <li class="list" @mouseenter="enter(index)" @mouseleave="leave(index)" v-for="(data, index) in msg"
            :key="index">
            ...
            <div v-for="(replayData, index2) in data.replays" :key="index2"
              style="background-color: #f5f5f6;padding: 20px;">
              <p class="comment"><i class="iconfont icon-huifuxiaoxi"></i>{{ replayData.replay }}</p>
              <p style="color: #ccc;">{{ replayData.replayTime }}</p>
            </div>
